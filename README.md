# 舞光戰將點燈計劃

> 展晟照明股份有限公司　業務部新人培訓互動 Demo
> THE GLOW UP PROGRAM · INTERNAL TRAINING USE

純靜態 HTML / CSS / JS 互動式網頁 demo，無後端依賴，可直接用 GitHub Pages 部署。

## 章節導覽

| 章節 | 檔案 | 內容 |
|------|------|------|
| 首頁 | [index.html](./index.html) | 7 大關卡地圖、品牌主張、徽章預覽 |
| CH 01 | [brand.html](./brand.html) | 品牌之光：39 年沿革時間軸、領航團隊、三大體驗館 |
| CH 02 | [products.html](./products.html) | 照明專家：800+ 產品、6 主打翻轉卡、規格速查 |
| CH 03 | [academy.html](./academy.html) | 業務學院：客戶分層、報價議價、5 大異議處理 |
| CH 04 | [challenge.html](./challenge.html) | 闖關挑戰：12 道情境題 + 8 輪 AI 客戶對話模擬 |
| CH 05 | [digital.html](./digital.html) | 數位行銷整合：6 觸點、新世代學院、O2O、ESG |
| 後台 | [admin.html](./admin.html) | 主管 CMS 原型：儀表板、學員、章節、題庫管理 |

## 設計風格

- **主色**：舞光橘 (`#F58220`) + 深炭黑底
- **字型**：思源黑體 / 思源宋體 + Inter
- **互動**：粒子背景、滾動激活、卡片翻轉、即時答題、AI 對話模擬

## 內容素材來源

公開資訊整理（今周刊、商業周刊、工商時報、myMKC、舞光官網等公開報導），業務話術為基於通用 B2B 經銷模式的演示版本，正式上線前需依公司實際 SOP 校對。

---
v0.5 PREVIEW · 內部 demo 階段
